...
/*
* Modbus library. It works kinda separately. Data read from and written to internal arrays. 
* Program should never use this data other than with modbus commands api.
* All important information should be copied to corresponding variables.
*/

uint16_t InputRegisters[InRegSize];
uint16_t InputRegister[InRegSize]; // added for 0x04 command

uint16_t OutputRegisters[OutRegSize];

uint8_t InputCoils[InCoilsSize];
uint8_t OutputCoils[OutCoilsSize];
/*----------------------------------------------------------------------------*/
/* Calculate CRC                                                              */
/*----------------------------------------------------------------------------*/
...
void Preset_Single_Register(uint8_t *RxData, uint8_t RxLength, uint8_t *TxData, uint8_t *TxLength)
{
	...
	{
		if((RegValue <= 0xFFFF))		/*(Псевдо проверка, возможно в будущем понадобится установить границы)*/
		{
	...
	if(StartAdr < OutRegSize)
	{
		if((RegCounter >= 1) && ((StartAdr + RegCounter) <= OutRegSize))
		{
			if(((RegCounter << 1) == ByteCounter) && ((ByteCounter + 7) == RxLength))
			{
				...
				RxLength -= ByteCounter + 1; // Response will not contain actual date and ByteCounter itself
				for(*TxLength = 2; *TxLength < RxLength; (*TxLength)++)		/*Формируем ответ как Эхо из запроса*/
				{
					TxData[*TxLength] = RxData[*TxLength];
				}
...
void ModBusRTU_PR(uint8_t *Receiver_arr, uint8_t c_Receiver_arr, uint8_t *Transceiver_arr, void (*ModBusRTU_CallBack)(int8_t)) /*Modbus RTU Processing and Responding*/
{
	uint16_t CRC_MB = 0;
	uint16_t CRC16 = 0;
